...
# session intent fit from the model
# session_intent = np.random.normal(0,1,session_num)
# youku
session_intent = np.array([0.2080,0.7340,0.3059,-0.1254,-0.7332,-0.1190,-0.6203,-0.1800,-0.2846,-1.2596])
# yelp
session_intent = np.array([0.6210,0.7225,-0.5768,-0.4739,-0.3096,0.4531,1.1926,-1.5080,-0.2468,0.4359])
# movielen
session_intent = np.array([0.3585,-0.6330,-0.6051,-0.1917,0.3688,0.3834,0.3537,0.1034,-0.1590,-0.0819])

# Simulate the first 10 item interactions as user history
user_records = np.zeros((num_user,time_range),dtype=np.int64)
reward_matrix = scipy.spatial.distance.cdist(user_embedding, item_embedding, metric='cosine')
scaler = MinMaxScaler()
